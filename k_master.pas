...
type
  ...
  function SendFundsFromAddress(Destination:String; AddressIndex,Restante:int64;concepto:string;comisionrestante:int64):int64;
...
while contador < (filesize(FilaWallet)) do
   ...
   WriteData.Custom:= ReadData.Custom;
...
While Restante > 0 do
   ...
   if StrToInt64(ArrayMyAddresses[contador].Balance) > 0 then
      begin
      MontoFromAddress := SendFundsFromAddress(Destination, contador, Restante, concepto, comisionrestante);
      Restante := Restante - MontoFromAddress;
      comisionrestante := comisionrestante - MontoFromAddress;
      if comisionrestante < 0 then comisionrestante := 0;
      end;
   contador := contador - 1;
   end;
...
if not VerifySignedString(TimeStamp+Sender+Destination+Monto+Concepto,SigHash,GetAddressPubKey(Sender)) then exit;
if proceder then
   begin
...
if Value < CONST_MinFee then value := CONST_MinFee;
result := value;
End;
...
var
  MontoRaiz : Int64;
Begin
if Monto < CONST_MinFee then result := 0
else if ((monto >= CONST_MinFee) and (monto < 10000*CONST_MinFee)) then result := CONST_MinFee
else
   begin
   MontoRaiz := (Monto*10000) div 10001;
   result := monto - montoraiz;
   end;
End;
...
SignedHash := GetStringSigned(TimeStamp+Sender+Destination+IntToStr(MontoFinal)+Concepto,ArrayMyAddresses[AddressIndex].PrivateKey);
